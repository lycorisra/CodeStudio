<template>
  <li :class="['level' + node.level]">
    ...
    <ul v-if='node.expand'>
        <ztree-item v-for="node in node.children" :node.sync="node" :trees.sync='trees' :key="node.name" :onNodeSelected="onNodeSelected"></ztree-item>
    </ul>
</li>
</template>
...
export default {
  ...
  methods: {
    ...
    nodeClick(node) {
      ...
      typeof this.onNodeSelected === "function" && this.onNodeSelected(node);
